...
I tried changing the underlying motion model in the GPS using the AVIONIC (high dynamic) mode via GPS.sendCommand("$PMTK886,2*2A"), but that made it worse the other way. I got ridiculous speeds in a short time frame that were clearly not right.

In the end, this GPS just could not accurately measure 0-60 speeds with accelerations for cars that are faster than 5.0 seconds. My Kia Stinger comes in around 4.5seconds, and I could just not get consistent readings.
